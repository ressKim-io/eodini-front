...
- [x] 역할별 홈 화면 구현 ✅ (12차 작업 완료)
- [ ] 권한 기반 접근 제어 (RBAC)
- [ ] 코드 생성 (build_runner) 실행 필요

---

### 12차: 역할 기반 홈 화면 구현 (2025-11-08)

#### 홈 화면 분리 전략
기존 홈 화면은 관리자용으로만 설계되어 있었음. 사용자 역할에 따라 보여지는 정보와 기능을 완전히 분리함.

**핵심 개념**: "내가 오늘 탈 차량이 어디에 있는가"
- 관리자: 모든 차량/운행 관리
- 사용자(학부모/일반): 배정된 차량과 운행 정보만 표시
- 운전자: 본인에게 배정된 운행과 탑승자만 표시

#### 구현된 화면

##### 1. AdminHomeScreen (관리자 홈 화면)
**파일**: `lib/features/home/screens/admin_home_screen.dart`
...
##### 2. ParentHomeScreen (보호자 홈 화면)
**파일**: `lib/features/home/screens/parent_home_screen.dart`
...
##### 3. PassengerHomeScreen (일반 회원 홈 화면)
**파일**: `lib/features/home/screens/passenger_home_screen.dart`
...
##### 4. DriverHomeScreen (운전자 홈 화면)
**파일**: `lib/features/home/screens/driver_home_screen.dart`
...
- ✅ 역할 기반 자동 라우팅
  ...
  - `UserRole.passenger` → `PassengerHomeScreen`
  - `UserRole.driver` → `DriverHomeScreen`
  - `UserRole.attendant` → `AdminHomeScreen` (TODO: 동승자 전용 화면)

#### Mock 데이터 특징
- ✅ **사용자/학부모**: 배정된 1대의 차량, 1명의 운전자만 표시
- ✅ **운전자**: 오늘 배정된 운행 목록 (등원/하원 각 1건씩)
- ✅ **탑승자 정보**: 의료 특이사항이 있는 경우 빨간색으로 강조
...
- ✅ 빠른 작업 그리드 (2열)
- ✅ 탑승/하차 상태 아이콘 표시
- ✅ 운행 상태 색상 구분 (예정/진행중/완료/취소)

#### 다음 단계
- [ ] 백엔드 API 연동 (실제 배정된 차량/운행 데이터 가져오기)
- [ ] 실시간 위치 페이지 역할별 필터링
- [ ] 운행 기록 페이지 역할별 필터링
- [ ] 동승자(Attendant) 전용 홈 화면 구현
- [ ] RBAC 구현 (라우터 가드 강화)
...
- [x] 자녀 정보 조회 (Mock)
- [x] 자녀의 운행 일정 조회 (Mock)
- [x] 자녀의 실시간 위치 추적 (배정된 차량) - UI 완료
- [x] 자녀의 탑승/하차 기록 조회 - UI 완료
- [x] 알림 설정 (탑승/하차 알림) - UI 완료
- [ ] 자녀 정보 수정 (제한적) - 백엔드 구현 필요

**일반 회원 (성인 탑승자)** ✅ 완료 (12차)
- [x] 본인 정보 조회 (Mock)
- [x] 본인의 운행 일정 조회 (Mock)
- [x] 본인의 실시간 위치 추적 (배정된 차량) - UI 완료
- [x] 본인의 탑승/하차 기록 조회 - UI 완료
- [x] 알림 설정 - UI 완료
- [ ] 본인 정보 수정 (제한적) - 백엔드 구현 필요

**운전기사 (Driver)** ✅ 완료 (12차)
- [x] 본인에게 배정된 운행 목록 조회 (Mock)
- [x] 운행 시작/종료 - UI 완료
- [x] 탑승자 탑승/하차 체크 - UI 완료
- [x] 배정된 경로 및 정류장 정보 조회 - UI 완료
- [ ] 본인 정보 조회/수정 (제한적) - 백엔드 구현 필요
- [x] 오늘의 운행 일정 (Mock)
